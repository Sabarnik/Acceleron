...
export const Route = createFileRoute("/services/detail/$slug")({
  component: SubServiceDetailRoute,
});

function SubServiceDetailRoute() {
  const { slug } = useParams({ from: "/services/detail/$slug" });
  const data = getSubServiceBySlug(slug);
    ...
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center bg-background text-foreground px-6 text-center">
        ...
        <p className="text-muted-foreground mb-8 max-w-md">
          The requested service page "{slug}" could not be found or has been moved.
        </p>
        <Link
          to="/services"
