...
import { CheckCircle, ArrowUpRight, Server } from "lucide-react";
...
function ITInfraPage() {
  const subServices = getSubServicesByCategory("it-infrastructure");
  ...
  return (
    <main className="relative min-h-screen overflow-hidden bg-background">
      <PageHero
        ...
        icon={<div className="flex h-16 w-16 items-center justify-center rounded-2xl bg-brand-gradient text-white shadow-glow"><Server className="h-8 w-8" /></div>}
      />

      <section className="container mx-auto px-6 max-w-7xl relative z-10 py-24">
        <Reveal>
          <div className="text-center mb-16">
            <h2 className="text-3xl font-bold text-foreground mb-4">Infrastructure Services</h2>
            <p className="text-muted-foreground max-w-2xl mx-auto">We design, deploy, and manage the critical infrastructure that powers your business.</p>
          </div>
        </Reveal>
      ...
      <section className="section-dark container mx-auto px-6 max-w-7xl relative z-10 py-24">
        <Reveal>
          <div className="text-center mb-16">
            <h2 className="text-3xl font-bold text-foreground mb-4">Infrastructure Lifecycle</h2>
            <p className="text-muted-foreground max-w-2xl mx-auto">Continuous assessment, optimization, and protection of your IT estate.</p>
          </div>
        </Reveal>
        <ProcessFlow
          steps={[
            { step: "01", title: "Audit", desc: "Infrastructure assessment and capacity planning" },
            { step: "02", title: "Design", desc: "Architecture blueprinting and vendor selection" },
            { step: "03", title: "Deploy", desc: "Implementation with zero-downtime migration" },
            { step: "04", title: "Manage", desc: "24/7 monitoring, support, and optimization" },
          ]}
        />
      </section>
      ...
      <section className="container mx-auto px-6 max-w-7xl relative z-10 py-24 text-center">
        <Reveal>
          <h2 className="text-3xl font-bold text-foreground mb-6">Modernize Your Infrastructure</h2>
          <Link to="/contact" className="inline-flex items-center gap-2 rounded-full bg-brand-gradient px-8 py-4 text-base font-bold text-white shadow-glow transition-all hover:scale-105">
            Get an Infrastructure Audit <ArrowUpRight className="h-5 w-5" />
          </Link>
        </Reveal>
      </section>
    </main>
